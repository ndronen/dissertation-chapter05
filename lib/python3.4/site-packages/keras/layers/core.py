...
class Merge(Layer):
    ...
    def __init__(self, layers, mode='sum', concat_axis=-1, dot_axes=-1):
        ...
        if mode not in {'sub', 'sum', 'mul', 'concat', 'ave', 'join', 'cos', 'dot'}:
            raise Exception('Invalid merge mode: ' + str(mode))

        if mode in {'sub', 'sum', 'mul', 'ave', 'cos'}:
    ...
    def output_shape(self):
        ...
        if self.mode in ['sub', 'sum', 'mul', 'ave']:
    ...
    def get_output(self, train=False):
        if self.mode == 'sum' or self.mode == 'ave':
            ...
            s = self.layers[0].get_output(train)
            return self.layers[0].get_output(train) - self.layers[1].get_output(train)
        elif self.mode == 'concat':
